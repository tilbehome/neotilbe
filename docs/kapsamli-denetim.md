...
S4 ek kök neden: ödeme görseli helper yerine `/theme/goldtheme/` adresini içeriyordu; paket içinde aynı dosya bulunmasına rağmen aktif tema kullanılıyordu. Görsel, boyut ve metin değiştirilmeden platform helper'ına taşındı. Böylece statik Twig kaynak sayısı 49'dan 50'ye, önceki taşınan bağlantı kanıt listesi 37'den 38'e çıktı; bunlar farklı kapsamlı sayımlardır. Ödeme güven metninin tüm etkin sağlayıcı/yöntemler için geçerliliği **mağaza ayarı/platform doğrulaması** bekler; kaynak dosyası eşliği ödeme sağlayıcısı veya metindeki beyanları doğrulamaz.
...
| S4 | header.twig ve birçok dosyada /theme/___shuttle URL | Mevcut varlığı helper’a bağla; olmayan dosyayı icat etme; ödeme iyzico görselinin goldtheme bağı da mevcut yerel aynı görsele taşındı | 37 bağlantıda onarım/yerel doğrulama; diğer dinamik/CSS yolları açık | tema-kaynak-baglari.json; onizleme-hazirligi.md; kapsamli-denetim.md sınır durumları |
