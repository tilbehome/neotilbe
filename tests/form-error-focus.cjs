...
(async () => {
    ...
    const retainedInput = 'İı O\'Neil "&" 🧺';
    await expr('document.querySelector("input[name=email]").value='+JSON.stringify(retainedInput));
    ...
    assert.equal(await expr('document.querySelector("input[name=email]").value'),retainedInput,'Validation failure must preserve input');
